...
class AppThemeProvider extends InheritedWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<DeviceTypeOverride>(
        'deviceTypeOverride',
        deviceTypeOverride,
      ),
    );
    ...
    properties.add(
      DiagnosticsProperty<ThemeModeOptionEnum>('themeMode', themeMode),
    );
    properties.add(DiagnosticsProperty<Locale>('locale', locale));
    properties.add(DiagnosticsProperty<String>('country', country));
    properties.add(
      DiagnosticsProperty<ConnectionMode>('connectionMode', connectionMode),
    );
  }

  static Widget buildAppThemeDropdown(
    BuildContext context,
    AppThemeEnum currentTheme,
    void Function(AppThemeEnum?) onChanged, {
    ...
    return AppDropdown<AppThemeEnum>(
      text: localizations.translate(currentTheme.storageKey),
      onPressed: () =>
          _showAppThemeSearchDialog(context, currentTheme, onChanged),
      icon: Icons.palette,
      theme: color,
      size: size,
      showTextAlways: true,
  ...
  static void _showAppThemeSearchDialog(
    BuildContext context,
    AppThemeEnum currentTheme,
    void Function(AppThemeEnum?) onChanged,
  ) {
      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            final filteredThemes = AppThemeEnum.values
                .where(
                  (theme) => theme.storageKey.toLowerCase().contains(
                  ...
                  children: [
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: localizations.translate('search_theme'),
                      ),
                    ),
                    Expanded(
                      child: ListView(
                        children: filteredThemes.map((theme) {
                          return ListTile(
                            title: Text(
                              localizations.translate(theme.storageKey),
                            ),
  ...
  static Widget buildThemeModeDropdown(
    BuildContext context,
    ThemeModeOptionEnum currentMode,
    ...
    return AppDropdown<ThemeModeOptionEnum>(
      ...
      onPressed: () =>
          _showThemeModeSearchDialog(context, currentMode, onChanged),
      theme: color,
  ...
  static void _showThemeModeSearchDialog(
    BuildContext context,
    ThemeModeOptionEnum currentMode,
    void Function(ThemeModeOptionEnum?) onChanged,
  ) {
    ...
    showDialog(
      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            final filteredModes = ThemeModeOptionEnum.values
                .where(
                ...
                child: Column(
                  children: [
                    TextField(
                      onChanged: (value) => setState(() => searchQuery = value),
                      decoration: InputDecoration(
                        hintText: localizations.translate('search_theme_mode'),
                      ),
                    ),
                    Expanded(
                      child: ListView(
                        children: filteredModes.map((mode) {
                          return ListTile(
                            title: Text(
                              localizations.translate('${mode.name}_theme'),
                            ),
  ...
  static Widget buildDeviceTypeOverrideDropdown(
    BuildContext context,
    DeviceTypeOverride currentOverride,
    void Function(DeviceTypeOverride?) onChanged, {
    String color = 'secondary',
    String size = 'm',
  }) {
    final localizations = AppLocalizations.of(context)!;
    ...
    return AppDropdown<DeviceTypeOverride>(
      ...
      onPressed: () => _showDeviceTypeOverrideSearchDialog(
        context,
        currentOverride,
        onChanged,
      ),
  ...
  static void _showDeviceTypeOverrideSearchDialog(
    BuildContext context,
    DeviceTypeOverride currentOverride,
    void Function(DeviceTypeOverride?) onChanged,
  ) {
    ...
    showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            final filteredOverrides = DeviceTypeOverride.values
                .where(
                  (override) => override.name.toLowerCase().contains(
                    searchQuery.toLowerCase(),
                  ),
                )
                .toList();
            ...
            return AlertDialog(
              title: Text(
                localizations.translate('select_device_type_override'),
              ),
              content: SizedBox(
                width: 300,
                height: 400,
